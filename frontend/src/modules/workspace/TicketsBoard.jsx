...
export default function TicketsBoard() {
  ...
  function createTicket(event) {
    ...
    setForm({ title: "", machineCode: "", clientName: "", priority: "NORMALE", description: "", technician: "" });
    setShowForm(false);
  ...
  return (
    <div className="space-y-5 p-5 md:p-8">
      ...
      {showForm ? (
        <Modal title="Créer un ticket SAV" onClose={() => setShowForm(false)}>
          <form className="space-y-4" onSubmit={createTicket}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Code machine"><Input value={form.machineCode} onChange={(event) => setForm({ ...form, machineCode: event.target.value })} placeholder="MC-2026-001" /></Field>
              <Field label="Client"><Input value={form.clientName} onChange={(event) => setForm({ ...form, clientName: event.target.value })} /></Field>
            ...
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => setShowForm(false)} className="rounded-2xl">Annuler</Button>
